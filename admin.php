...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch($action) {
            case 'add_donor':
                $stmt = $conn->prepare("INSERT INTO donors (donor_id, name, email, blood_type, organ, status) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([
                    $_POST['donor_id'],
                    $_POST['name'],
                    $_POST['email'],
...
<body>
    <div class="app">
        ...
        <aside class="sidebar" aria-label="Main navigation">
            <div>
                <div class="brand">
                <img src="logo.png" alt="LifeLink Logo" style="width: 44px; height: 44px; border-radius: 8px;">
                <h3>LifeLink</h3>
                </div>
            ...
            <div class="logout-wrap">
                ...
                <form method="POST" style="width:100%;">
                    <!-- If you have a real logout action, replace with actual logic -->
                    <button type="button" class="logout-btn" onclick="alert('Implement logout logic or link to your logout endpoint')">LOGOUT</button>
                </form>
            </div>
        </aside>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= htmlspecialchars($donation['organ']) ?></td>
                                <td><span class="status-badge status-<?= strtolower($donation['status']) ?>"><?= htmlspecialchars($donation['status']) ?></span></td>
                                <td><?= htmlspecialchars($donation['date_requested']) ?></td>
                                <td><?= htmlspecialchars($donation['date_updated'] ?? '-') ?></td>
                                <td class="action-buttons">
